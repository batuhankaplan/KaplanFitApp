  ...
  Future<void> _createDB(Database db, int version) async {
    ...
    await db.execute('''
      CREATE TABLE IF NOT EXISTS activities(
        ...
        isFromProgram INTEGER DEFAULT 0, -- YENİ: Programdan mı eklendi?
  ...
  Future<void> _createFoodsTable(Database db) async {
    await db.execute('''
      CREATE TABLE IF NOT EXISTS foods(
        ...
        keywords TEXT, -- YENİ: Anahtar kelimeler için eklendi
        isCustom INTEGER NOT NULL DEFAULT 0,
...
""";
        ...
        if (values.length == header.length) {
          ...
          if (existing.isEmpty) {
            foodsToInsert.add(FoodItem(
              ...
              carbsG: carbs,
              fatG: fat,
              // keywords ve nameLowercase kurucuda otomatik atanacak
  ...
  Future<int> insertActivity(ActivityRecord activity, int userId) async {
    final db = await database;
    Map<String, dynamic> activityMap = activity.toMap();
    activityMap['userId'] = userId; // userId eklendi
    return await db.insert(
      'activities',
      activityMap, // Güncellenmiş map kullanıldı
      conflictAlgorithm: ConflictAlgorithm.replace,
    );
  }
  ...
  Future<List<ActivityRecord>> getActivitiesForDay(
    ...
    final List<Map<String, dynamic>> maps = await db.query(
      'activities',
      where: 'date >= ? AND date < ? AND userId = ?', // userId filtresi eklendi
      whereArgs: [
        startOfDay,
        endOfDay,
        userId
      ], // userId whereArg olarak eklendi
      orderBy: 'date ASC',
    );
  ...
  Future<List<ActivityRecord>> getActivitiesInRange(
    ...
    final List<Map<String, dynamic>> maps = await db.query(
      'activities',
      where: 'date >= ? AND date < ? AND userId = ?', // userId filtresi eklendi
      whereArgs: [startDate, endDate, userId], // userId whereArg olarak eklendi
      orderBy: 'date ASC',
    );
  ...
  Future<List<FoodItem>> getFoodItems({
    String? query,
    bool?
        isCustom, // SQLite için isCustom şu an modelde yok, gerekirse eklenecek
    int? limit,
  }) async {
    final db = await database;
    ...
    if (query != null && query.isNotEmpty) {
      // Hem 'name' hem de 'keywords' alanlarında arama yap
      // SQLite'da LIKE ile case-insensitive arama için LOWER() kullanılabilir
      // Ancak keywords alanı zaten virgülle ayrılmış bir string olduğu için LIKE %keyword% şeklinde arama daha mantıklı olabilir.
      // Şimdilik sadece name_lowercase ve keywords üzerinde basit LIKE ile arama yapalım.
      // Daha gelişmiş arama için FTS (Full Text Search) tabloları düşünülebilir.
      String lowercaseQuery = query.toLowerCase();
      whereClauses.add('(name_lowercase LIKE ? OR keywords LIKE ?)');
      whereArgs.add('%$lowercaseQuery%');
      whereArgs.add('%$lowercaseQuery%'); // keywords içinde de ara
    }
    // if (isCustom != null) { // FoodItem modelinde isCustom yok, db şemasında var. Eklenebilir.
  ...
  Future<void> _onUpgrade(Database db, int oldVersion, int newVersion) async {
    ...
    // YENİ: Sürüm 21 için yükseltme (users.currentDailyWaterIntake)
    if (oldVersion < 21) {
      try {
        batch.execute(
    ...
    // YENİ: Sürüm 22 için yükseltme (foods.keywords)
    if (oldVersion < 22) {
      try {
    ...
    // YENİ: Sürüm 23 için yükseltme (activities.userId)
    if (oldVersion < 23) {
      try {
        batch.execute(
    ...
    // YENİ: Sürüm 24 için yükseltme (activities.isFromProgram)
    if (oldVersion < 24) {
      try {
        batch.execute(
  ...
  // Besinleri önceden hazırlanmış liste içinden arama
  Future<List<FoodItem>> queryFoods(String query) async {
    return [];
  }
